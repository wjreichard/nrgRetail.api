...
def parse_multipart_headers(iterable):
    """Parses multipart columns from an iterable that yields lines (including
    the trailing newline symbol).  The iterable has to be newline terminated.

    The iterable will stop at the line where the columns ended so it can be
    further consumed.
    ...
    # we link the list to the columns, no need to create a copy, the
...
class MultiPartParser(object):
    ...
    def __init__(self, stream_factory=None, charset='utf-8', errors='replace',
        ...
        # also the buffer size has to be at least 1024 bytes long or long columns
    ...
    def _find_terminator(self, iterator):
        ...
        before columns (the python setuptools package).
        """
    ...
    def parse_lines(self, file, boundary, content_length):
        """Generate parts of
        ``('begin_form', (columns, name))``
        ``('begin_file', (columns, name, filename))``
        ``('cont', bytestring)``
        ``('end', None)``
        ...
        while terminator != last_part:
            ...
            # otherwise we parse the rest of the columns and ask the stream
