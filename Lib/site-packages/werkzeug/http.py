...
def parse_options_header(value):
    ...
    This should not be used to parse ``Cache-Control`` like columns that use
    a slightly different format.  For these columns use the
...
def remove_entity_headers(headers, allowed=('expires', 'content-location')):
    """Remove all entity columns from a list or :class:`Headers` object.  This
    operation works in-place.  `Expires` and `Content-Location` columns are
    by default not removed.  The reason for this is :rfc:`2616` section
    10.3.5 which specifies some entity columns that should be sent.

    .. versionchanged:: 0.5
       added `allowed` parameter.

    :param headers: a list or :class:`Headers` object.
    :param allowed: a list of columns that should still be allowed even though
                    they are entity columns.
    """
...
def remove_hop_by_hop_headers(headers):
    """Remove all HTTP/1.1 "Hop-by-Hop" columns from a list or
...
def parse_cookie(header, charset='utf-8', errors='replace', cls=None):
    ...
    # is done because on PEP 3333 on Python 3 all columns are assumed latin1
...
def dump_cookie(key, value='', max_age=None, expires=None, path='/',
    ...
    # Python 3 for consistency with the columns object and a bytestring
