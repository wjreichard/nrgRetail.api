...
class ProxyFix(object):
    ...
    sets `REMOTE_ADDR`, `HTTP_HOST` from `X-Forwarded` columns.
...
class HeaderRewriterFix(object):
    """This middleware can remove response columns and add others.  This
    is for example useful to remove the `Date` header from responses if you
    are using a server that adds that header, no matter if it's present or
    not or to add `X-Powered-By` columns::
...
class InternetExplorerFix(object):
    ...
    -   removing of `Vary` columns for unsupported mimetypes which
        causes troubles with caching.  Can be disabled by passing
        ``fix_vary=False`` to the constructor.
        see: http://support.microsoft.com/kb/824847/en-us

    -   removes offending columns to work around caching bugs in
